...
type FormChangePassword = yup.InferType<typeof schema>;
...
const ChangePassword = () => {
  ...
  const onSubmit = handleSubmit(async (data) => {
    try {
      ...
      if (isAxiosUnprocessableEntityError<ErrorResponse<FormChangePassword>>(error)) {
        const formError = error.response?.data.data;
        if (formError) {
          Object.keys(formError).forEach((key) => {
            setError(key as keyof FormChangePassword, {
              message: formError[key as keyof FormChangePassword],
              type: 'Server'
            });
          });
        }
